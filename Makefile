...
$(OUT)compile_time_request.o: $(patsubst %.c, $(OUT)src/%.o.ctr,$(src-y)) ./scripts/buildcommands.py
	...
	$(Q)$(PYTHON) ./scripts/buildcommands.py \
	    $(if $(FORCE_VERSION),--force-version "$(FORCE_VERSION)",) \
	    -d $(OUT)klipper.dict -k $(OUT)defconfig -t "$(CC);$(AS);$(LD);$(OBJCOPY);$(OBJDUMP);$(STRIP)" $(OUT)compile_time_request.txt $(OUT)compile_time_request.c
